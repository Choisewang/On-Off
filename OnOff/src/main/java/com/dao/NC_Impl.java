...
public class NC_Impl implements Dao_NC {
	...
	public List<MyDto_NC> selectList() {
		...
		ResultSet rs = null;
		List<MyDto_NC> list = new ArrayList<MyDto_NC>();		
		try {
			...
			while(rs.next()) {
				MyDto_NC dto = new MyDto_NC();
				dto.setC_no(rs.getInt(1));
				dto.setN_no(rs.getInt(2));
